...
void Sys_SendPacket(int length, const void *data, const netadr_t *to)
{
	...
	{
		// browsers can't send UDP: datagrams go over the WebXDC realtime
		// channel instead (net_webxdc.c)
		qboolean NET_WebxdcSend(int length, const void *data, const netadr_t *to);

		if (NET_WebxdcSend(length, data, to))
		{
			return;
		}
	}
...
#ifdef __EMSCRIPTEN__
	{
		// dispatch packets received over the WebXDC realtime channel
