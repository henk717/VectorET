...
static void *Sys_TryLibraryLoad(const char *base, const char *gamedir, const char *fname)
{
	...
	// a failed dlopen() poisons Emscripten's loaded-library cache by name:
	// a later dlopen of the same path "succeeds" but exposes no symbols.
	// Only attempt to load files that actually exist.
	if (access(fn, R_OK) != 0)
	{
		Com_Printf("failed: file does not exist\n");
		return NULL;
	}
#endif
...
/**
 * @brief Main game loop - browser variant.
 *
 * The browser owns the event loop: hand Com_Frame to requestAnimationFrame
 * instead of spinning. emscripten_set_main_loop() unwinds the stack and
 * never returns (simulate_infinite_loop = 1).
 */
