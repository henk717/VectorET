...
/**
 * @brief Teach SDL that a browser gamepad is a game controller.
 *
 * SDL's Emscripten joystick driver returns SDL_FALSE from
 * GetGamepadMapping, so a pad the browser already reports in the "standard"
 * layout is never recognised as an SDL_GameController. Without that, the
 * engine falls back to the raw-joystick path and loses analog stick look,
 * which is the whole point of using a pad here.
 *
 * The W3C standard gamepad layout is fixed, so the mapping is known ahead of
 * time; only the GUID varies, and that is read back per device.
 *
 * https://w3c.github.io/gamepad/#remapping
 */
static void IN_RegisterWebGamepadMappings(int total)
{
	...
	for (i = 0; i < total; i++)
	{
		...
		{
			continue; // SDL already knows this one
		}

		id = SDL_JoystickGetDeviceGUID(i);
		SDL_JoystickGetGUIDString(id, guid, sizeof(guid));
		...
		// commas would split the mapping into bogus fields
		Com_sprintf(mapping, sizeof(mapping), "%s,%s,%s", guid, "Web Gamepad", layout);

		if (SDL_GameControllerAddMapping(mapping) < 0)
		{
			Com_Printf(S_COLOR_YELLOW "Could not map web gamepad '%s': %s\n",
			           name, SDL_GetError());
		}
		else
		{
			Com_Printf("Mapped web gamepad '%s' to the standard layout\n", name);
		}
	}
}
#endif
...
static void IN_InitJoystick(void)
{
	...
	// registered once: hot-plug re-runs this whole function per device event
...
#ifdef __EMSCRIPTEN__
	IN_RegisterWebGamepadMappings(total);
#endif
...
static void IN_ProcessEvents(void)
{
	...
	while (SDL_PollEvent(&e))
	{
...
#ifdef __EMSCRIPTEN__
				// Without pointer lock the real cursor stays visible and the
				// player compares it against the one cgame draws. Those deltas
				// are window pixels, but cgame keeps its cursor in the 640x480
				// virtual screen and clamps it there, so on any other canvas
				// size the two travel at different rates - and once the game's
				// cursor clamps at an edge the offset never recovers.
				//
				// Virtual height is always 480 and virtual width follows the
				// aspect, so one uniform factor of 480/windowHeight puts both
				// cursors in step and makes them reach every edge together.
				if (!SDL_GetRelativeMouseMode() && cls.glconfig.windowHeight > 0)
				{
					static float residualX = 0.0f, residualY = 0.0f;
					float        scale     = (float)SCREEN_HEIGHT / cls.glconfig.windowHeight;
					int          dx, dy;

					// carry the fraction: scaling down would otherwise round
					// slow, deliberate aiming away to nothing
					residualX += e.motion.xrel * scale;
					residualY += e.motion.yrel * scale;
		...
		case SDL_JOYDEVICEREMOVED:
			// Joysticks are only enumerated once at startup, which is fine on a
			// desktop where the pad is already plugged in. A browser hides pads
			// until the user presses a button on one, so on the web the device
			// essentially always arrives late - without this, a controller is
