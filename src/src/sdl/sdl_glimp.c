...
#ifdef __EMSCRIPTEN__
		// Request a GLES 3.0 context: Emscripten's SDL maps it to WebGL2
		// (the build disallows WebGL1 via MIN_WEBGL_VERSION=2). gl4es then
		// translates the renderer's desktop GL onto it.
		SDL_GL_SetAttribute(SDL_GL_CONTEXT_PROFILE_MASK, SDL_GL_CONTEXT_PROFILE_ES);
...
#ifdef __EMSCRIPTEN__
		// gl4es is statically linked with NO_LOADER/NO_INIT_CONSTRUCTOR:
		// feed it the GLES symbol resolver and initialize it before any GL call.
		setenv("LIBGL_USEVBO", "0", 1);
		// NOT SDL_GL_GetProcAddress: Emscripten's eglGetProcAddress hands out
		// non-FULL_ES2 passthrough functions (precompiled system lib), which
		// silently break client-side vertex arrays. See sdl_gl4es_procs.c.
		{
			extern void *ETL_GLES2_GetProcAddress(const char *name);
			set_getprocaddress(ETL_GLES2_GetProcAddress);
		}
		initialize_gl4es();

		// The browser canvas only ever has one WebGL context: on vid_restart
		// the same context object comes back with its previous state while
		// the GL JS library's binding shadows may disagree. Resync the buffer
		// bindings through the (statically linked, emulated) GLES2 API.
		// Vertex attrib enables are intentionally left alone: gl4es' hardware
		// state mirror survives the restart and would go out of sync.
		{
			extern void *ETL_GLES2_GetProcAddress(const char *name);
			typedef void (*etl_bindbuf_t)(unsigned int, unsigned int);
			etl_bindbuf_t etl_glBindBuffer = (etl_bindbuf_t)ETL_GLES2_GetProcAddress("glBindBuffer");
			if (etl_glBindBuffer)
			{
				etl_glBindBuffer(0x8892 /* GL_ARRAY_BUFFER */, 0);
				etl_glBindBuffer(0x8893 /* GL_ELEMENT_ARRAY_BUFFER */, 0);
			}
		}
#endif
